...
#if RCT_DEV || RNDVP// Debug executors are only supported in dev mode
...
- (void)setUp
{
  if (!_url) {
    ...
    NSString *URLString;
    NSString *ip = [[NSUserDefaults standardUserDefaults]stringForKey:@"DebugServerIP"];
    if ([ip length] > 0) {
      URLString = [NSString stringWithFormat:@"http://%@:%zd/debugger-proxy?role=client",ip, port];
    }else{
      URLString = [NSString stringWithFormat:@"http://localhost:%zd/debugger-proxy?role=client", port];
    }
